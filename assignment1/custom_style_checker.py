...
class FunctionInfo(StyleBase):
    ...
    def has_annotations(self) -> bool:
        # Check if each argument has an annotation and if there's a return type hint
...
class CodeAnalyzer:
    ...
    def get_func_info(self, item: ast.FunctionDef) -> FunctionInfo:
        annotations = [arg.annotation for arg in item.args.args if arg.arg != "self"]  # Skip 'self'
    ...
    def get_return_type(self, func_item) -> str:
        ...
        return_type = None
        for stmt in ast.walk(func_item):  # Recursively find return statements
